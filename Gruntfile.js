...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    uglify: {
      ...
      build: {
        src: './src/lunametrics-vimeo.gtm.js',
        dest: './lunametrics-vimeo.gtm.min.js'
      }
    },
    fixConfig: {
      options: {
        build: {
          src: './lunametrics-vimeo.gtm.min.js',
          dest: './lunametrics-vimeo.gtm.min.js'
        }
      }
    },
    ...
    updateContainer: {
      options: {
        build: {
          src: './lunametrics-vimeo.gtm.js',
          dest: './luna-vimeo-tracking.json'
        }
      }
    }
  });
  ...
  grunt.registerTask('updateContainer', ['Updating container import file'], function() {
    ...
    for (i = 0; i < oldContainer.containerVersion.tag.length; i++) {
      ...
      if (oldContainer.containerVersion.tag[i].name === 'CU - Vimeo Tracking - LunaMetrics Plugin') {
    ...
    oldContainer.containerVersion.tag[oldTag].parameter[oldParameter].value = '<script type="text/javascript" id="gtm-vimeo-tracking">\n' +
      newScript +
      '\n</script>'
